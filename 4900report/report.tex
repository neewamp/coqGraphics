...
It shouldn't be hard to see the merits in writing proofs that live at a level above the implementation.  One of the first proofs that we wrote for the Coq implementation, specifically using maps of type : $point \rightarrow color$, was reasonable to complete at a similar level.  The Theorem simply states that:

\begin{theorem}[Draw Pixel Correct]
\forall (t : Type) p1 p2 p


\end{theorem}

if two points are drawn using draw\_pixel  An invariant about the draw pixel function was very easy to add into the class. It was then easy to use it to prove 
